...
def get_holdings(pk):
   SQL = '''SELECT ticker_symbol, number_of_shares FROM holdings WHERE 
account_pk = ?'''
   values = (pk,)
   cursor.execute(SQL, values)
...
def get_holding(pk, ticker_symbol):
  #connection = sqlite3.connect(‘example.db’, check_same_thread = False)
  #cursor = connection.cursor()
  sql = '''SELECT number_of_shares FROM holdings WHERE account_pk = ? and
ticker_symbol = ?'''
...
#return false if shit doesn't work, otherwise return true
...
def sell(account_pk, ticker_symbol, number_of_shares):
   ...
   if number_of_current_shares < number_of_shares:
       return False
   else:
